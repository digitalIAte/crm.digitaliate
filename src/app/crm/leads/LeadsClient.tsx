        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-3 items-center bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                <input
                    ...
                    className="flex-1 min-w-48 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-digitaliate/30"
                />
                ...
                <div className="flex items-center gap-2 text-sm text-gray-500">
                    ...
                    <input type="range" min={0} max={100} step={5} value={minScore} onChange={e => setMinScore(Number(e.target.value))}
                        className="w-24 accent-digitaliate" />
                    <span className="font-bold text-gray-700 w-6">{minScore}</span>
                </div>
                <button onClick={() => exportToCSV(filtered)}
                    className="ml-auto flex items-center gap-1.5 bg-digitaliate text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-digitaliate-dark transition-colors shadow-sm">
                    ⬇︎ Exportar CSV
                </button>
            </div>
            ...
            {/* Bulk action bar */}
            {selected.size > 0 && (
                <div className="flex items-center gap-3 bg-digitaliate/10 border border-digitaliate/20 px-4 py-3 rounded-xl text-sm font-medium text-digitaliate-dark">
                        ...
                        <tr>
                            <th className="px-4 py-3 w-10">
                                <input type="checkbox" checked={allSelected} onChange={toggleAll}
                                    className="rounded accent-digitaliate cursor-pointer" />
                            </th>
                            ...
                            <tr key={lead.id} className={`hover:bg-slate-50 transition-colors ${selected.has(lead.id) ? "bg-blue-50/50" : ""}`}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <div className="flex items-center gap-3">
                                        <div className="h-8 w-8 rounded-full bg-gradient-to-br from-digitaliate to-blue-400 flex items-center justify-center text-white font-bold text-sm shadow-sm">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <div className="flex items-center gap-2">
                                        <div className="w-14 bg-gray-200 rounded-full h-1.5">
                                            <div className="h-1.5 rounded-full bg-digitaliate" style={{ width: `${lead.score}%` }} />
                                        </div>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-right">
                                    <Link href={`/crm/leads/${lead.id}`}
                                        className="text-digitaliate font-medium hover:text-digitaliate-dark bg-blue-50 px-3 py-1.5 rounded-lg hover:bg-blue-100 text-sm transition-colors">
                                        Ver →
                                    </Link>
                                </td>
                            </tr>
