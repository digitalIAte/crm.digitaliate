            ...
            {loading ? (
                ...
                <ul className="space-y-3 mb-6">
                    {reminders.map(r => {
                        ...
                        return (
                            <li key={r.id} className={`flex items-start gap-3 p-3 rounded-lg border transition-colors ${r.done ? 'bg-gray-50 border-gray-100 opacity-60' : isOverdue ? 'bg-red-50/50 border-red-100' : 'bg-white border-gray-200 hover:border-digitaliate/30'}`}>
                                <input
                                    type="checkbox"
                                    checked={r.done}
                                    onChange={() => toggleReminder(r.id, r.done)}
                                    className="mt-1 h-4 w-4 rounded border-gray-300 text-digitaliate focus:ring-digitaliate"
                                />
                                <div className="flex-1">
                                    <p className={`text-sm ${r.done ? 'line-through text-gray-500' : 'text-gray-900 font-medium'}`}>
                                        {r.text}
                                    </p>
                                    <p className={`text-xs mt-1 font-medium ${r.done ? 'text-gray-400' : isOverdue ? 'text-red-600' : 'text-digitaliate'}`}>
                                        {isOverdue && !r.done && "⚠️ "}
            ...
            <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200/60">
                ...
                <div className="space-y-3">
                    <input
                        ...
                        className="w-full text-sm border-gray-200 rounded-md py-2 focus:ring-digitaliate"
                    />
                    <div className="flex gap-2">
                        <input
                            ...
                            className="flex-1 text-sm border-gray-200 rounded-md py-2 focus:ring-digitaliate"
                        />
                        <button
                            onClick={addReminder}
                            disabled={!text || !date || isPending}
                            className="bg-digitaliate text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-digitaliate-dark disabled:opacity-50 shadow-sm"
                        >
                            Guardar
                        </button>
                    </div>
