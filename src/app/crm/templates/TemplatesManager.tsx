                    ...
                    <form onSubmit={handleSave} className="space-y-4">
                        ...
                        {channel === "email" && (
                            <div>
                                ...
                                <input required value={subject} onChange={e => setSubject(e.target.value)} type="text" className="w-full border-gray-200 rounded-lg text-sm" placeholder="Ej: Propuesta de servicios DigitalIAte" />
                            </div>
                        )}

                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Cuerpo del mensaje</label>
                            <p className="text-xs text-gray-400 mb-2">Puedes usar variables como <code className="bg-gray-100 px-1 rounded text-digitaliate">{'{{name}}'}</code>, <code className="bg-gray-100 px-1 rounded text-digitaliate">{'{{email}}'}</code> que se auto-rellenarán.</p>
